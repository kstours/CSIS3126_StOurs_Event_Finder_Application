...
object User : Authentication() {
    ...
    fun setUserInformation(username:String,token:String,information: HashMap<String,String>,interests:ArrayList<String>){
        databaseReference = FirebaseDatabase.getInstance().getReference("Users")
        databaseReference.child(username).get().addOnSuccessListener { it ->
            if (it.exists()) {
                databaseReference.child(username).child("token").get().addOnSuccessListener {
                    if (it.exists()) {
                        if(it.value.toString() == token){
                            ...
                            databaseReference = FirebaseDatabase.getInstance().getReference("Users")
                            databaseReference.child(username).child("information").setValue(information)
    ...
    fun changePassword(username: String, password: String, newPw: String,confirmNewPW:String,token:String, context: Context) {
                ...
                databaseReference.child(username).child("password").get()
                    .addOnSuccessListener { it ->
                        ...
                        if (result.verified) {
                            ...
                            User.username = username //store username that way i can access data later
                            val intent = Intent(context, HomeActivity::class.java)
                            Event.getEventData()
                            information = dataSnapshot.child("information").getValue<HashMap<String,String>>()!!
                            interests = dataSnapshot.child("interests").getValue<ArrayList<String>>()!!
                            context.startActivity(intent) //if its right  login
                        } else {
                            Toast.makeText(context, "Token Mismatch", Toast.LENGTH_SHORT).show()
                        }
                    }
            }
        }
